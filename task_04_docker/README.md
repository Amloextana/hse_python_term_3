...
Для сборки перейдите в директорию с проектом и запустите сборку
```bash
cd task_04_docker/
docker-compose up --build -d
```
флаг -d нужен для запуска в фоне  
флаг --build нужен при первой сборке или при необходимости пересборки (после изменения кода)

После успешной сборки на :8889 порту развернется streamlit приложение, а на :8888 python-backend


**Требования**  
Для работы приложения должны быть установлены **Docker** и **Docker Compose**

**Особенности**  
При сборке код так же проходит проверку flake8, в случае несоответсвия, сборка не завершится